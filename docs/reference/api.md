...
(optional, object) Inline bootstrap configuration that will be created when the schedule is active.
Mutually exclusive with `bootstrapDataSecretName` — exactly one must be set.
...
(optional, string, non-empty) Name of an existing Secret containing bootstrap data
(e.g., an OpenShift worker ignition config). When set, 5-Spot skips creating a
bootstrap resource and sets `Machine.spec.bootstrap.dataSecretName` directly.
Mutually exclusive with `bootstrapSpec` — exactly one must be set.

Use this for OpenShift clusters where the worker ignition config is fetched from
the Machine Config Server and stored in a pre-existing Secret.
